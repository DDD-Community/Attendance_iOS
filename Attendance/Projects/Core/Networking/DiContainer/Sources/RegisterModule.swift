...
public struct RegisterModule {
  ...
  public func makeRepository<T, U>(
    _ protocolType: T.Type, factory: @escaping () -> U
  ) -> () -> Module {
    return {
      self.makeRepositoryModule(protocolType) {
  ...
  // MARK: -  usecase 생성
  public func makeUseCase<T, U>(_ protocolType: T.Type, factory: @escaping () -> U) -> () -> Module {
    return {
      self.makeUseCaseModule(protocolType) {
  ...
  // MARK: -  usecase 모듈 생성
  private func makeUseCaseModule<T>(
    _ type: T.Type,
    factory: @escaping () -> T
  ) -> Module {
    self.makeModule(type, factory: factory)
