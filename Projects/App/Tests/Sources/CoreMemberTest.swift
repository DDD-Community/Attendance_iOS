...
struct CoreMemberTest {
  ...
  struct AttendanceTest {
      ...
      testStore.assert { state in
        var updatedAttendanceCheckInModel = state.attendanceCheckInModel.map { attendance in
          ...
          if modifiedAttendance.roleType == .android, modifiedAttendance.roleType == .iOS {
            #expect(
              modifiedAttendance.status == .notAttendance,
              "Expected Android attendance status to be .notAttendance"
            )
        ...
        #expect(
          updatedAttendanceCheckInModel == state.attendanceCheckInModel,
          "Expected attendance model to be updated accordingly"
        )
    ...
    @MainActor func member_출석날짜를현재날짜말고_다른날짜로변경() async throws {
      guard let selectDateSecond = "2024-09-16".date(as: .yearMonthDay) else {
...
//            await testStore.receive(.async(.fetchMemberDataResponse(.success(testStoreMemmberModel))), timeout:
//            .seconds(1)) {
