...
actor TokenRefreshManager {
  ...
  func refreshCredentialIfNeeded() async throws -> AccessTokenCredential {
    // 간단히 토큰 갱신 수행
    if isRefreshing {
      // 다른 작업이 진행 중이면 잠시 대기 후 재시도
      try await _Concurrency.Task.sleep(nanoseconds: 100_000_000) // 0.1초 대기
...
/// 🚀 AsyncMoya 기반 토큰 관리 인터셉터 + RequestInterceptor 호환
final class AuthInterceptor: RequestInterceptor, @unchecked Sendable {
  private let tokenRefreshManager = TokenRefreshManager()

  /// AsyncMoya 요청에 토큰 자동 추가
  ...
  // MARK: - RequestInterceptor Implementation
