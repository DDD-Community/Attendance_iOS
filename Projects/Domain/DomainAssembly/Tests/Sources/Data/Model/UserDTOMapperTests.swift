...
struct UserDTOMapperTests {
  ...
  func mapsApiEnumsToDomainEnums() {
    let dto = UserDTO(
      ...
      managing: OnBoardingDomainInterface.Managing.attendanceCheck,
      role: OnBoardingDomainInterface.SelectPart.web,
      memberTeam: OnBoardingDomainInterface.SelectTeam.ios1,
      staffRole: Staff.manager
    )

    let user = dto.toDomain()

    #expect(user.userRole == UserRole.member)
    #expect(user.managing == StaffManaging.attendanceCheck)
    #expect(user.role == SelectParts.frontend)
    #expect(user.memberTeam == SelectTeams.ios1)
    #expect(user.staffRole == Staff.manager)
  }
